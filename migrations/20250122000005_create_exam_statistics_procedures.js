...
exports.up = async function(knex) {
    ...
    // Lưu ý: Syntax này là PostgreSQL, MySQL/MariaDB không hỗ trợ PL/pgSQL
    // Sẽ bỏ qua tạo procedure này vì database sử dụng MySQL/MariaDB
    try {
        await knex.raw(`
        ...
        // Bỏ qua lỗi nếu database không hỗ trợ PL/pgSQL (MySQL/MariaDB)
        console.log('Skipping PostgreSQL functions (database is MySQL/MariaDB):', error.message);
    }
};
...
exports.down = async function(knex) {
    ...
    try {
        ...
        console.log('Skipping trigger drop (not applicable for this database)');
    }
    
    try {
        await knex.raw('DROP FUNCTION IF EXISTS update_exam_skill_statistics();');
    } catch (e) {
        console.log('Skipping function drop (not applicable for this database)');
    }
